...
function Strong_haste(cid, guid)
	setExhaustHealing(guid, 1000)
...
function useBestHealing(cid, guid)
	...
	local voc = getPlayerVocation(cid) -- Replace with the proper function to get the player's vocation
	local hasParalyze = getCreatureCondition(cid, CONDITION_PARALYZE) -- Replace with the proper function to check if the player has CONDITION_PARALYZE
	local minDelay = 500
	local maxDelay = 1500
	...
	if(voc == 1 or voc == 2) then
		...
		if hasParalyze then
			--Strong_haste(cid, guid)


			local randomDelay = math.random(minDelay, maxDelay) -- Generate a random delay between minDelay and maxDelay
			addEvent(function() Strong_haste(cid, guid) end, randomDelay) -- Schedule someFunction to be called after randomDelay milliseconds, passing the cid argument
			
			--local randomDelay = math.random(minDelay, maxDelay) -- Generate a random delay between minDelay and maxDelay
			--addEvent(Strong_haste(cid, guid), randomDelay)
...
-- adding paralyze rune




local paralyze_rune = createCombatObject()
setCombatParam(paralyze_rune, COMBAT_PARAM_TARGETCASTERORTOPMOST, true)
setCombatParam(paralyze_rune, COMBAT_PARAM_TYPE, COMBAT_EARTHDAMAGE)
setCombatParam(paralyze_rune, COMBAT_PARAM_EFFECT, CONST_ME_MAGIC_RED)
setCombatParam(paralyze_rune, COMBAT_PARAM_DISTANCEEFFECT, CONST_ANI_SUDDENDEATH)

local condition = createConditionObject(CONDITION_PARALYZE)
setConditionParam(condition, CONDITION_PARAM_TICKS, 20000)
setConditionFormula(condition, -0.9, 0, -0.9, 0)
setCombatCondition(paralyze_rune, condition)

setCombatFormula(paralyze_rune, COMBAT_FORMULA_LEVELMAGIC, -1, -60, -1, -60, 5, 5, 4, 7)




-- Add the magic wall rune's combat object and related functions
local magic_wall_combat = createCombatObject()
setCombatParam(magic_wall_combat, COMBAT_PARAM_DISTANCEEFFECT, CONST_ANI_ENERGY)
setCombatParam(magic_wall_combat, COMBAT_PARAM_CREATEITEM, 1497)
...
--[[
function findPath(start, goal, cid)
    local path = astar.compute(start, goal, function(a, b) return isSightClear(a, b, 1) and (not isPathable(a) or isCreature(cid)) end)
    if path then
        return path
    else
        return false
...
function castBestAttackingSpell(cid, guid)
    ...
    local friendOnScreen = false

    -- Check if there is a friend on the screen
    ...
    -- If there is a friend on screen, proceed with targeting and attacking
    if friendOnScreen then
        if not exAggr then
            -- Add the missing parts of the function here
            -- For example: conditions and actions for other vocations and scenarios


			
            if voc == 2 then
                local distance = getDistanceBetween(ppos, targetPos)

                -- Check if the target is running away (distance > some threshold)
                if distance > 4 and not exItem and getCreatureMana(cid) >= 1400 then
					doCombat(cid, paralyze_rune, target) -- Assuming 'combat' is the paralyze rune's combat object
					doCreatureAddMana(cid, -1400, false)
					setExhaustItem(guid, 2000) -- 2-second exhaust
				end
			end	
		
		
			if(voc == 1 or voc == 2 or voc == 3 or voc == 4) then
			local spectators = getSpectators(ppos, 7, 7, false)
			local targetCount = 0
			for _, creature in ipairs(spectators) do
				if isPlayer(creature) and getCreatureTarget(creature) == cid then
					targetCount = targetCount + 1
				end
			end
	
			if targetCount > 1 and not exItem and mana >= 210 and getDistanceBetween(targetPos, ppos) <= 7 and isSightClear(targetPos, ppos, 1) then
				-- Helper function to get positions around a given position
				local function getPositionsAround(pos)
					return {
				...
				-- Get positions around the player and the target
				local playerPositions = getPositionsAround(ppos)
				local targetPositions = getPositionsAround(targetPos)
	
				-- Find a suitable position for the magic wall
				local toPos
				for _, pPos in ipairs(playerPositions) do
					for _, tPos in ipairs(targetPositions) do
						if isSightClear(pPos, tPos, 1) then
							toPos = tPos
							break
						end
					end
					if toPos then break end
				end
	
				-- Create the magic wall
				if toPos and doCombat(cid, magic_wall_combat, positionToVariant(toPos)) then
					addEvent(mwallCounter, 1000, math.random(14, 18), toPos)
					doCreatureAddMana(cid, -210, false) -- Subtract the mana cost
					setExhaustItem(guid, 1000) -- Set the exhaust time for the magic wall rune
				end
			end
			
			 --[[
			--- walk through walls
			if getDistanceBetween(ppos, targetPos) > 7 then
				local direction = getDirectionTo(ppos, targetPos)
				if direction ~= -1 then
 ...
 --]]
			...
			-- Move closer to the target if too far away
		--[[	if getDistanceBetween(ppos, targetPos) > 7 then
				local path = findPath(ppos, targetPos, cid)
				if path and #path > 1 then
					local nextPos = path[2]
					local direction = getDirectionTo(ppos, nextPos)
					if direction ~= -1 then
						doMoveCreature(cid, direction)
					end
				end
...
--]]
            ...
            if(voc == 1) then
					...
					for _, guyInRange in pairs(getSpectators(ppos, 5,5,false)) do
						if(isPlayer(guyInRange)) then
							if(isBot(guyInRange) and math.abs(getPlayerGUID(guyInRange) - guid) < friendGUIDrange) then
								...
								foes = foes + 1
								print("my foes is:", foes)
							end
						end
					end
					if(friends < foes and friends < 6) then
				...
				if(not exItem and not exAggr and getDistanceBetween(targetPos, ppos) <= 7 and isSightClear(targetPos, ppos, 1)) then
					--doMoveCreature(cid,getDirectionTo(ppos,targetPos))
